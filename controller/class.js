    ...
    try {
        if (school_level == 'junior') { dept = null || undefined }
            ...
            if (dept == undefined || null) {
                return res.status(400).json({ error: "Department Cannot be undefined" })
            } dept = req.body.dept
        };
        ...
        let classTeacher = await ClassTeacher.findOne({
            $and: [{ dept: req.body.dept }, { class_name }, { class_teacher: class_teacher }]
        });
        if (teacherPresent || classTeacher) {
            return res.status(400).json({ error: "Class Teacher Already assigned" })
        }
